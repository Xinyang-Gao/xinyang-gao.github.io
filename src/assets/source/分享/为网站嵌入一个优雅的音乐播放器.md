...
!!!WARNING "[WARNING] 内容过时"
  本网站目前使用的是 [APlayer](https://github.com/DIYgod/APlayer/pull/802)。

!!!IMPORTANT "[IMPORTANT] 信息补充"
  本文章介绍的 [NeteaseMiniPlayer v2](https://github.com/numakkiyu/NeteaseMiniPlayer) 已停止维护，原作者现在维护 [NeteaseMiniPlayer v3](https://github.com/numakkiyu/NeteaseMiniPlayer_v3)。

!!!IMPORTANT "[IMPORTANT] 改版说明"
  本文的 NeteaseMiniPlayer v2 经过我的修改，增加了拖拽移动功能，原版并没有此项功能，[详见此 PR](https://github.com/numakkiyu/NeteaseMiniPlayer/pull/15)。

## 为什么要加一个播放器？

虽说音乐不是必需品，但它能提升氛围感。

我决定在网站上加入一个悬浮音乐播放器，最好支持播放网易云歌单，不干扰浏览，且能在页面切换时持续播放。经过一番搜索，我发现了开源项目 **NeteaseMiniPlayer V2** （NMPv2），由 `BHCN STUDIO & 北海的佰川` 开发。轻量、美观、功能完整，且提供了比较灵活的配置方式。
...
这篇文章记录了我如何把它和自制的无刷新导航，包括动态加载、样式适配、与现有主题联动，以及解决常见的坑。

---

## 一、选定播放器

市面上有很多第三方音乐插件，但有的太重（需要引入整个 UI 库），有的依赖外部服务器，有的无法自定义样式。NeteaseMiniPlayer 可以：
...
项目的 GitHub 地址：[github.com/numakkiyu/NeteaseMiniPlayer](https://github.com/numakkiyu/NeteaseMiniPlayer)
