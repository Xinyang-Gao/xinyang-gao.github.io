...
## v8.45.2 (2026-09-26)
- fix: 修复无刷新导航不支持浏览器前进和后退

## v8.45.1 (2026-09-26)
- fix(footer): 修复页脚“今日访问 / 累计访问”始终显示 0、趋势图一直转圈
...
- style(footer): 数据卡片右下角的装饰图标改为半透明（0.45，悬停 0.68），并抽出 `--footer-stat-icon-opacity` / `--footer-stat-icon-opacity-hover` 两个变量便于调整

## v8.45.0 (2026-09-26)
- fix(articles): 修复文章列表所有条目都显示“发布于 未指定日期”
...
- feat(articles): 文章 frontmatter 新增可选字段 `cover`，填写后作为列表项背景图显示
- feat(works): 作品 `metadata.json` 新增可选字段 `cover`（卡片背景）与 `archived`（卡片右上角标记“已归档 · 不再维护”）
- refactor(list): 文章列表项的发布日期 / 更新日期并入元信息行（作者 · 字数 · 阅读时长 · 发布于 · 更新于），圆点分隔、窄屏自动换行，移除 `.article-dates-top-right`
 - common: 新增 `UNKNOWN_DATE_TEXT` 与 `is_known_date()`，缺失日期整块省略，作品卡片同理不再渲染“未指定日期”
- docs: README 数据格式规范补充 `cover` / `archived` 字段说明

## v8.44.1 (2026-09-26)
- chore: 优化一些CSS

## v8.44.0 (2026-09-26)
- feat: 更新日志分片以减少每次加载的更新日志json大小
- chore: FA Kit 移入 `<head>` 并添加 defer

## v8.43.0 (2026-09-26)
- fix(frontend): 修复多处影响正常使用与资源回收的缺陷
...
- perf(frontend): 消除滚动与导航期的重复计算与重复下载
...
- refactor(frontend): 收敛重复实现并补齐类型与异常兜底
...
## v8.42.0 (2026-09-26)
- refactor(builder): 非构建资源改为声明式统一管理
...
## v8.41.1 (2026-09-26)
- style: 在导航栏和页脚添加 GaoXinYang.svg 及动画

## v8.41.0 (2026-09-26)
- refactor(builder): 现代化构建系统并修复增量与跨平台缺陷
...
## v8.40.0 (2026-09-26)
- feat: 添加了一个 SVG 绘制动画组件

## v8.39.3 (2026-09-26)
- feat: 在页脚添加开源致谢

## v8.39.2 (2026-09-25)
- refactor: 重写 github-contrib-graph 样式和 tooltip 使其适配网站

## v8.39.1 (2026-09-25)
- refactor: 优化 tooltip 切换动画

## v8.39.0 (2026-09-25)
- refactor: 重写关于页

## v8.38.2 (2026-09-25)
- style: 移除页脚没什么作用的探索板块并替换为版权信息

## v8.38.1 (2026-09-25)
- refactor: 简洁化隐私政策和404页

## v8.38.0 (2026-09-25)
- feat(image-viewer): 新增 FLIP 打开/关闭动画并优化查看器性能

## v8.37.4 (2026-09-24)
- refactor(mouse-effects): 重构自定义光标以对齐 cursor-fx 3.0.0 并优化性能

## v8.37.3 (2026-09-24)
- chore: 更新 twikoo 版本（1.7.24 -> 2.0.9）并适配相应的类名变化
 - JS 大小减少约 20Kb
 - CSS 大小减少约 8Kb

## v8.37.2 (2026-09-19)
- fix: 计算最后更新时间未把代码更新计入

## v8.37.1 (2026-09-19)
- style: 美化页脚布局
- feat: 将网站页脚的流量计数显示从 [Vercount](https://www.vercount.one) 改为 [FakeICP](https://fakeicp.top)

## v8.37.0 (2026-09-19)
- refactor: 重构加载覆盖层，优化动画和布局

## v8.36.2 (2026-09-18)
- fix: 修复 theme-controller.ts 未导入 Utils 导致生产环境无法进入网站的 BUG
- chore: 更新 twikoo 版本（1.7.22 -> 1.7.24）
...
## v8.35.2 (2026-09-12)
- refactor(builder): 现代化构建引擎并修复并行与增量缺陷
...
## v8.35.0 (2026-09-12)
- refactor(core): 统一工具函数与导航事件，消除重复实现
...
## v8.34.0 (2026-09-12)
- refactor: 初始化/清理/设置/主题/单例 五项统一重构
...
## v8.33.1 (2026-09-12)
- chore: 更新 twikoo 版本（1.7.20 -> 1.7.22）

## v8.33.0 (2026-09-12)
- refactor(stats): 图表逻辑迁移至注册表
...
- refactor(data): SW 与 DataService 缓存职责分离，移除 localStorage 层
 - BREAKING CHANGE:
...
- perf(search): 移除 searchWorker，改为同步过滤排序
...
## v8.32.0 (2026-09-12)
- refactor: 抽取 theme-controller 与 modal-base，引入 DisposableStack 和 ScrollDispatcher
...
## v8.30.3 (2026-09-11)
- refactor(timeline): 时间线改为按日期卡片分组展示
...
## v8.30.2 (2026-09-06)
- style: 在页脚新增由 custom-icon-badges.demolab.com 提供的 Badge 图片

## v8.30.1 (2026-08-29)
- feat: 优化友链颜色提取算法，避免极端颜色
...
## v8.30.0 (2026-08-29)
- fix: 修复 `CookieConsentManager` 并未被实例化导致 `storageController` 一直处于禁用状态，导致所有 `setItem` 被忽略，`getItem` 返回 `null` 的 BUG
 - 此修复也连带将欢迎覆盖层的一系列关于存储的问题给修复了（包括一段时间内刷新网页会再次显示覆盖层的问题）
- feat: 将很多直接调用 `localStorage` 的代码改为统一使用 `storageController`

## v8.29.1 (2026-08-29)
- style: 跳转欢迎覆盖层的更新日志显示，现在看起来更美观清楚了，排序改为又新到旧
- fix: 修复上个版本设置面板打开就刷新背景图，以及一些设置写入读写问题

## v8.29.0 (2026-08-29)
- feat: 添加更多设置功能并优化设置面板布局

## v8.28.1 (2026-08-29)
- fix: 修复友链复制信息中含多余空格的 BUG
- chore: 在友链页完善我的网站信息，优化一些 tooltip 的显示

## v8.28.0 (2026-08-29)
- feat: 使 APlayer 适配网站主题，支持拖拽、顺序折叠动画

## v8.27.3 (2026-08-28)
- fix: 彻底修复 tooltip 文本测量误差导致显示时可能出现一个字符被迫换行超出背景的情况

## v8.27.2 (2026-08-28)
- fix: 移除多余的 friends-manager 全局调用

## v8.27.1 (2026-08-27)
- fix: ~~修复 tooltip 文本测量误差导致显示时可能出现一个字符被迫换行超出背景的情况~~（实际上这次修复几乎没有解决此问题——来自2026年8月28日）
- chore: 优化友链 `本站用到的工具和服务` 板块

## v8.27.0 (2026-08-27)
- feat: 新增 tooltip
- chore: 使用 tooltip 精简设置面板文字

## v8.26.10 (2026-08-27)
- chore: 删除友链页冗余部分，优化文本

## v8.26.9 (2026-08-27)
- style: 微调深浅模式切换按钮中太阳的图标样式和切换动画

## v8.26.8 (2026-08-27)
- style: 同步 Github 提示框样式

## v8.26.7 (2026-08-26)
- style: 同步文章和作品页的筛选便签样式同步首页标签云，并支持根据数量多少调整大小

## v8.26.6 (2026-08-26)
- chore: 优化关于页代码，便于维护

## v8.26.5 (2026-08-25)
- chore: 更新 twikoo 版本（1.7.19 -> 1.7.20）

## v8.26.4 (2026-08-14)
- refactor: 重构twikoo样式，基于新的 1.7.19 原版改版替换原 1.7.15 原版改版
 - 为新增的搜索功能添加样式
 - 增加与网站样式的统一性
 - 修复标签输入弹窗显示异常的问题
 - 修复暗色模式下一些文本显示异常的问题
...
## v8.26.3 (2026-08-14)
- chore: 更新 twikoo 版本（1.7.15 -> 1.7.19）

## v8.26.2 (2026-08-13)
- style: 友链页的互换友链部分使用更合理的布局，调整一些文本样式

## v8.26.1 (2026-08-13)
- refactor: 重构统计页样式，优化一些条目的显示

## v8.26.0 (2026-08-12)
- perf: 移除 `stats-init.ts` 包装层，让 `Router` 直接实例化 `FullStatsManager`
- fix: 修复统计页面缺失`<div id="router-view">`导致无刷新导航失效的问题
- chore: 一些文本修改

## v8.25.4 (2026-08-12)
- fix: 修复时间线页面同时间多版本排序错误问题
- feat: 时间线页面新增筛选文章、作品、更新日志选择功能和搜索功能
- feat: 为回到顶部按钮增加进度条

## v8.25.3 (2026-08-12)
- fix: 修复特定窗口宽度下时间线更新日志详情内容布局过窄

## v8.25.2 (2026-08-12)
- style: 简化时间线页面中网站更新日志的显示

## v8.25.1 (2026-08-12)
- fix: 补全构建 `works.json`、`friends.json`、`friend_colors.json`

## v8.25.0 (2026-08-12)
- feat: 合并归档与更新日志为统一时间线页面
...
## v8.24.0 (2026-08-12)
- feat(ui): 统一弹窗组件为通用 `detail-dialog`
...
## v8.23.1 (2026-08-03)
- refactor: 重构 加载覆盖层 的样式，减少冗余代码

## v8.23.0 (2026-08-03)
- refactor: 重构 `router.ts` 修复一些BUG，优化逻辑，减少极端情况下报错的概率
*已知问题：无刷新导航对浏览器的前进/后退无法相应（技术问题，之后修复）*

## v8.22.3 (2026-08-03)
- refactor: 重构设置面板，移除多余的文本和元素，调整了布局

## v8.22.2 (2026-08-03)
- feat: 为文章查看页添加 markdown admonition 扩展语法解析
- chore: 清理构建系统多余代码
...
## v8.22.0 (2026-07-28)
- refactor: 大幅重构 `mouse-effects.ts` 的 `MouseEffectManager` 类，以优化性能和可维护性
- feat: 在 `mouse-effects.ts` 中添加基于[ Cursor Fx 脚本](https://github.com/Xinyang-Gao/cursor-fx-userscript) `WEB API` 的判断：若用户已加载此脚本，则不再初始化网站自身的鼠标特效，以避免视觉冲突和性能影响

## v8.21.3 (2026-07-28)
- refactor: 优化文章查看页样式，统一风格，减少代码量（减少约 200 行）

## v8.21.2 (2026-07-28)
- chore: 优化图片查看器代码，增加可维护性

## v8.21.1 (2026-07-28)
- docs: 更新隐私政策

## v8.21.0 (2026-07-28)
- refactor: 重构twikoo样式，基于新的 1.7.15 原版改版替换原 1.7.7 原版改版
 - 增加与网站样式的统一性
 - 大幅减少代码量（减少约 1700 行）

## v8.20.0 (2026-07-28)
- refactor: 重构图片查看器，重构更现代简约的页面并优化代码质量，有一定性能提升

## v8.19.0 (2026-07-28)
- refactor: 重构网站首页

## v8.18.0 (2026-07-28)
- refactor: 重构网站基础样式，减少硬编码，统一风格，向简约方向重构，减少代码量(约400行CSS)

## v8.17.0 (2026-07-28)
- refactor: 重构导航栏

## v8.16.1 (2026-07-27)
- chore: 更新 twikoo 版本（1.7.14 -> 1.7.15）

## v8.16.0 (2026-07-27)
- refactor: 重构鼠标特效，复用[ cursor-fx-userscript 的部分代码](https://github.com/Xinyang-Gao/cursor-fx-userscript)

## v8.15.1 (2026-07-27)
- chore: 将[Github贡献图JS](https://unpkg.com/github-contrib-graph@latest/dist/browser.global.js)替换为本地文件，以减少外部网络不稳定造成的加载缓慢
- style: 重构（简约方向）页脚的样式，减少冗余CSS

## v8.15.0 (2026-07-27)
- style: 重构（简约方向）关于页的样式，减少冗余CSS，修复深色模式下关于页评论区透明的问题

## v8.14.2 (2026-07-23)
- style: 将个人信息卡片和外链跳转的头像图片样式同步为友链卡片头像的样式

## v8.14.1 (2026-07-22)
- docs: 规范更新日志格式，并使一些版本、图片和缩进能被正确解析

## v8.14.0 (2026-07-22)
- feat: 统一数据服务并全面重构缓存与请求层
...
## v8.13.1 (2026-07-21)
- chore: 将从 `https://vercount.one/js` 获取计数 JS 改为获取 `/js/vendor/vercount.min.js` 用以规避一些加载延迟

## v8.13.0 (2026-07-21)
- refactor(entry): 拆分 `main.ts` 启动流程，引入加载覆盖层与初始化编排器
...
## v8.12.3 (2026-07-21)
- refactor(article): 将文章阅读页的 TOC 目录改为在构建时生成

## v8.12.2 (2026-07-21)
- refactor(settings): 将设置面板内联样式迁移至 CSS，优化布局结构

## v8.12.1 (2026-07-21)
- feat: 将 works 迁出主仓库将其链接为子模块

## v8.12.0 (2026-07-18)
- refactor(router): 重构无刷新导航，解决内存泄漏、资源累积
