...
import { CONFIG, type Config } from '/js/core/core.js';
...
class FullStatsManager {
    ...
    private async fetchAllData(): Promise<void> {
        const endpoints: Array<{ key: keyof FullStatsManager['data']; url: string }> = [
    ...
    private renderCoreCards(): void {
        ...
        const totalWords =
            stats.total_word_count ?? this.articlesList.reduce((sum, a) => sum + (a.word_count || 0), 0);
        ...
        const avgTagPerArticle =
            totalArticles
                ? (
                    this.articlesList.reduce((sum, a) => sum + (a.tags?.length || 0), 0) / totalArticles
                ).toFixed(1)
                : '—';

        const codeDensity =
            totalFiles !== '—' && totalLines !== '—' && totalFiles > 0 ? Math.round(totalLines / totalFiles) : '—';
    ...
    `;
        ...
        document.getElementById('extraTotalWords') &&
            (document.getElementById('extraTotalWords')!.innerText = totalWords.toLocaleString());
    }
    ...
    private renderExtraDetails(): void {
        ...
        if (listContainer) {
            if (sortedByWords.length) {
                listContainer.innerHTML = sortedByWords
                    .map(
                        (art) =>
                ...
                listContainer.innerHTML = '<li>暂无数据</li>';
            }
        }
        ...
        if (totalReadSpan) {
            totalReadSpan.innerText =
                totalReadMins > 0 ? `${totalReadMins} 分钟 (≈${(totalReadMins / 60).toFixed(1)}h)` : '—';
        }

        // 代码规模深度
        const code = this.data.codeAnalysis || {};
        document.getElementById('codeTotalFiles') &&
            (document.getElementById('codeTotalFiles')!.innerText = String(code.total_files ?? '—'));
        document.getElementById('codeNonEmptyLines') &&
            (document.getElementById('codeNonEmptyLines')!.innerText = String(
                code.non_empty_lines ?? code.total_lines ?? '—'
            ));

        const totalSizeKB = code.total_size_bytes ? (code.total_size_bytes / 1024).toFixed(1) : '—';
        document.getElementById('codeTotalSize') &&
            (document.getElementById('codeTotalSize')!.innerText = String(totalSizeKB));

        const extensions = code.by_extension || [];
        let topExt = extensions.length
            ? extensions.sort((a, b) => b.count - a.count)[0].extension
            : '—';
        document.getElementById('codeTopExt') &&
            (document.getElementById('codeTopExt')!.innerText =
                topExt === 'no_extension' ? '无后缀' : topExt);
        ...
        const avgHtml = extAvg
            .map((e) => `<span class="badge-stat">${e.name}</span> ${e.avg}行/文件`)
            .join(' &nbsp; ');
        document.getElementById('extAvgLinesList') &&
            (document.getElementById('extAvgLinesList')!.innerHTML = avgHtml || '—');

        // 作者数量
        const authors = new Set(this.articlesList.map((a) => a.author).filter(Boolean));
        document.getElementById('authorCount') &&
            (document.getElementById('authorCount')!.innerText = String(authors.size));

        // 最近更新月份
        const lastActive = this.getLatestActiveMonth();
        document.getElementById('lastActiveMonth') &&
            (document.getElementById('lastActiveMonth')!.innerText = lastActive);

        const stats = this.data.statistics || {};
        document.getElementById('totalUpdateDays') &&
            (document.getElementById('totalUpdateDays')!.innerText = String(stats.total_update_days ?? '—'));
        document.getElementById('statsUpdatedDate') &&
            (document.getElementById('statsUpdatedDate')!.innerText = stats.last_updated
                ? `· 版本 ${stats.version || '—'}  · ${stats.last_updated}`
                : '');
        document.getElementById('snapshotTime') &&
            (document.getElementById('snapshotTime')!.innerText = stats.last_updated_full
                ? new Date(stats.last_updated_full).toLocaleString()
                : '未知');
        document.getElementById('totalArticlesStat') &&
            (document.getElementById('totalArticlesStat')!.innerText = String(this.articlesList.length));
        document.getElementById('totalWorksStat') &&
            (document.getElementById('totalWorksStat')!.innerText = String(this.worksList.length));
    }
    ...
    private getLatestActiveMonth(): string {
    ...
    private renderAllCharts(): void {
        ...
        this.renderCodeLineShareChart();
    }

    private getChartColors(): { textColor: string; gridColor: string; accent: string } {
    ...
    private renderTrendChart(): void {
        ...
        if (data.length) {
            ...
            if (maxIdx !== -1) {
                document.getElementById('peakMonthLabel') &&
                    (document.getElementById('peakMonthLabel')!.innerText =
                        labels[maxIdx] + ` (${data[maxIdx]}篇)`);
            }
        }
    }
    ...
    private renderArticleTagsChart(): void {
        ...
        if (!tags.length) {
            ...
            this.articlesList.forEach((art) => {
                (art.tags || []).forEach((t) => tagMap.set(t, (tagMap.get(t) || 0) + 1));
            });
        ...
        const labels = top10.map((t) => t.name);
        ...
        const chart = new Chart(ctx, {
            ...
            data: {
                labels,
                datasets: [{ label: '引用次数', data, backgroundColor: accent, borderRadius: 6 }],
            },
            options: {
                indexAxis: 'y',
                responsive: true,
    ...
    private renderWorkTagsChart(): void {
        ...
        const labels = workTags.map((t) => t.name);
        const data = workTags.map((t) => t.count);
        const { textColor } = this.getChartColors();
    ...
    private renderCodeExtensionChart(): void {
    ...
    private renderWordHistogram(): void {
    ...
    private renderWorksYearChart(): void {
        const canvas = document.getElementById('worksYearChart') as HTMLCanvasElement | null;
        if (!canvas) return;
    ...
    private renderCodeLineShareChart(): void {
        ...
        let top = valid
            .sort((a, b) => (b.non_empty_lines || 0) - (a.non_empty_lines || 0))
            .slice(0, 6);
