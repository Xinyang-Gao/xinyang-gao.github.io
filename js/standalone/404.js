...
(function() {
  ...
  const CONFIG = {
    ...
    defaultSuggestions: [
      ...
      { text: '浏览全部文章', url: '/articles.html', icon: 'fa-book-open' },
      { text: '创意作品集', url: '/works.html', icon: 'fa-cube' },
  ...
  class MessageGenerator {
    ...
    generateSmartMessage() {
      ...
      if (isArticleMatch) {
        ...
        suggestions = [
          { text: '浏览全部文章', url: '/articles.html', icon: 'fa-book-open' },
          { text: '热门标签导航', url: '/articles.html?tags=', icon: 'fa-tags' }
        ];
        if (probableSlug) {
        ...
        suggestions = [
          { text: '进入作品集', url: '/works.html', icon: 'fa-cube' },
          { text: '创意实验室', url: '/works.html?tags=实验', icon: 'fa-flask' }
        ];
        if (probableSlug) {
        ...
        suggestions = [
          { text: '全部文章标签云', url: '/articles.html', icon: 'fa-tag' },
          { text: '创意工坊标签', url: '/works.html', icon: 'fa-palette' }
        ];
        if (tagName) {
        ...
        suggestions = [
          { text: '留言板', url: '/contact.html', icon: 'fa-comment-dots' },
          { text: '返回首页', url: '/', icon: 'fa-house' }
        ];
      } else {
        ...
        suggestions = [
          ...
          { text: '作品精选', url: '/works.html', icon: 'fa-rocket' }
        ];
  ...
  class UI404Manager {
    ...
    bindQuickSearch() {
      ...
      const performSearch = () => {
        ...
        window.location.href = query ? `/articles.html?search=${encodeURIComponent(query)}` : '/articles.html';
      };
