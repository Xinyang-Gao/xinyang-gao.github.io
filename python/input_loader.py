...
# 导入公共模块
from common import (
    PROJECT_ROOT, SOURCE_DIR, WORKS_ROOT, JSON_OUTPUT_DIR, HTML_OUTPUT_DIR,
...
def parse_changelog(md_text: str) -> Dict:
    """解析更新日志，返回 {version_str: {'date': str, 'changes': list}}，自动去重"""
...
def load_version(force: bool = False) -> Dict:
    """加载 version.json，若更新日志有变化则重新生成"""
    changelog_path = PROJECT_ROOT / "assets" / "网站更新日志.md"
...
def _process_markdown_file(md_file_path: Path, old_article: Optional[Dict] = None, category: str = None) -> Article:
    """处理单个 Markdown 文件，返回 Article 对象，并生成对应的 HTML 文件。"""
...
def load_articles(force_refresh: bool = False) -> List[Article]:
    """从 source 目录加载所有 Markdown，返回 Article 对象列表。支持增量缓存。"""
